...
pub mod reader {
    ...
    impl FWT {
        /// Build a frame waiting time from its `PP`, `MM` and `DD` components.
        ///
    ...
    mod tests {
        ...
        pub fn test_fwt() {
            ...
            assert!(FWT::new(15, 0, 0).is_none());
            assert!(FWT::new(16, 0, 0).is_none());
            assert!(FWT::new(u8::MAX, 0, 0).is_none());
            assert!(FWT::new(0, 0, 128).is_none());
            assert_eq!(FWT::new(0, 0, 0).unwrap().us(), 302.06488);
            assert_eq!(FWT::new(MAX_PP, 0, 0).unwrap().us(), 4949031.0);
        ...
        pub fn test_parameters_max_pp() {
            ...
            assert_eq!(
                Parameters::default()
                    .fwt(FWT::new(MAX_PP, 0, 0).unwrap())
                    .data(),
                ([0x00, 0x0E, 0x00, 0x00, 0x03, 0xFF, 0x00, 0x1A], 8),
            );
        }
    }
}
